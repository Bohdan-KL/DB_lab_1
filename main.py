...
def createConnection():
    for attempt in range(10):
        try:
            connection = psycopg2.connect(dbname='student01_DB', user='postgres', password='root1', host='localhost')
...
def loadDataIntoDB(df, year, connection):
    ...
    try:
        cursor = year.cursor()
        cursor.execute("SELECT COUNT(*) FROM zno_records WHERE year=" + str(connection))
        num_of_records_to_ignore = cursor.fetchall()[0][0]

    except psycopg2.OperationalError:
        year = createConnection()
        loadDataIntoDB(df, year, connection)

    counter = 0
    for row in df.values:
        row = list(row)
        row.insert(0, connection)
        cash.append(row)
